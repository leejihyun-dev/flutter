...
void main() {
  ...
  testUsingContext(
      'ResidentRunner can handle an RPC exception from debugFrameJankMetrics',
      () => testbed.run(() async {
            fakeVmServiceHost =
                FakeVmServiceHost(requests: <VmServiceExpectation>[
              listViews,
            ...
            expect((globals.logger as BufferLogger).warningText,
                contains('Unable to get jank metrics for Impeller renderer'));
          }, overrides: <Type, Generator>{
            Usage: () => TestUsage(),
          }));
  ...
  testUsingContext(
      ...
      () => testbed.run(() async {
            ...
            final List<Event> newEventList = fakeAnalytics.sentEvents
                .where((Event e) => e.eventName == DashEvent.hotRunnerInfo)
                .toList();
            ...
            arbFile.writeAsStringSync('''
{
  ...
  testUsingContext(
      ...
      () => testbed.run(() async {
            ...
            ddsLauncherCallback = (Uri uri,
              ...
              throw FakeDartDevelopmentServiceException(
                message:
            ...
            runZonedGuarded(() {
              flutterDevice
                  .connect(allowExistingDdsInstance: true)
                  .then((_) => done.complete());
            }, (Object e, StackTrace st) {
              expect(e, isA<ToolExit>());
              expect(
                  (e as ToolExit).message,
                  contains(
                    'Existing VM service clients prevent DDS from taking control.',
                  ));
              done.complete();
  ...
  testUsingContext(
      'Falls back to existing DDS URI from exception message',
      () => testbed.run(() async {
            fakeVmServiceHost =
                FakeVmServiceHost(requests: <VmServiceExpectation>[]);
            final FakeDevice device = FakeDevice()
            ...
            await done.future;
            expect(device.dds.uri,
                Uri.parse('http://localhost/existingDdsInMessage'));
          }, overrides: <Type, Generator>{
            VMServiceConnector: () => (
                  Uri httpUri, {
                  ReloadSources? reloadSources,
  ...
  testUsingContext(
      ...
      () => testbed.run(() async {
            ...
            ddsLauncherCallback = (Uri uri,
                ...
                Uri? serviceUri,
                List<String> cachedUserTags = const <String>[],
                dds.UriConverter? uriConverter}) async {
              expect(uri, Uri(scheme: 'foo', host: 'bar'));
  ...
  testUsingContext(
      'Context includes URI converter',
      () => testbed.run(() async {
            fakeVmServiceHost =
                FakeVmServiceHost(requests: <VmServiceExpectation>[]);
            final FakeDevice device = FakeDevice()
...
class FakeDartDevelopmentService extends Fake
    implements dds.DartDevelopmentService {
  @override
...
class FakeDartDevelopmentServiceException
    implements dds.DartDevelopmentServiceException {
...
class FakeProjectFileInvalidator extends Fake
    implements ProjectFileInvalidator {
  @override
  Future<InvalidationResult> findInvalidated({
    ...
    return InvalidationResult(packageConfig: packageConfig, uris: <Uri>[
      Uri.parse('file:///hello_world/main.dart'),
    ]);
  }
}
